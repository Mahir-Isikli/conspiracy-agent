...
actor TokenService {
    ...
    func fetchConnectionDetails(roomName: String, participantName: String) async throws -> ConnectionDetails? {
        ...
        if let localConnectionDetails = try await fetchConnectionDetailsFromLocal(roomName: roomName, participantName: participantName) {
            return localConnectionDetails
        }
    ...
    private let localServerUrl: String = "https://jopyter.com"
    
    private func fetchConnectionDetailsFromLocal(roomName: String, participantName: String) async throws -> ConnectionDetails? {
        var urlComponents = URLComponents(string: "\(localServerUrl)/getToken")!
        ...
        do {
            ...
            guard let httpResponse = response as? HTTPURLResponse else {
                debugPrint("Failed to connect to local token server")
                return nil
            }
            
            guard (200 ... 299).contains(httpResponse.statusCode) else {
                debugPrint("Error from local token server: \(httpResponse.statusCode)")
                return nil
            }
            
            struct LocalTokenResponse: Codable {
                let token: String
                let serverUrl: String
                let roomName: String
                let participantName: String
            ...
            guard let tokenResponse = try? JSONDecoder().decode(LocalTokenResponse.self, from: data) else {
                debugPrint("Error parsing token response from local server")
                return nil
            }
            ...
            debugPrint("Error connecting to local token server: \(error)")
            return nil
        }
    }
